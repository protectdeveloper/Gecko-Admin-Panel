...
function calculateEventPosition(event: CalendarEventType, allEvents: CalendarEventType[]): EventPosition {
  ...
  // Aynı zamanda başlayan etkinlikler için özel işlem
  const sameTimeEvents = allEvents.filter((e) => e.start.getTime() === event.start.getTime() && isSameDay(e.start, event.start));

  if (sameTimeEvents.length > 1) {
    // Aynı zamanda başlayan etkinlikler için
    const sameTimePosition = sameTimeEvents.indexOf(event);
    const width = `${Math.max(70, 100 / sameTimeEvents.length)}%`;
    const leftOffset = sameTimePosition * 15; // Her etkinlik için 15px sağa kaydır
  ...
  // Normal çakışan etkinlikler için mevcut mantık
  ...
  return (
    <MotionConfig reducedMotion="user">
      <AnimatePresence mode="wait">
        ...
        >
          <motion.div
            className={cn(`flex flex-col w-full text-${event.color}-500`, month && 'flex-row items-center justify-between')}
            layout="position"
          >
            <p className={cn('font-bold truncate', month && 'text-xs')}>{event.title}</p>
            <p className={cn('text-sm', month && 'text-xs')}>
              <span className={cn(month && 'hidden')}>{format(event.start, 'HH:mm')}</span>
              <span className={cn('mx-1', month && 'hidden')}>-</span>
              <span className={cn(month && 'hidden')}>{format(event.end, 'HH:mm')}</span>
            </p>
          </motion.div>
        </motion.div>
      </AnimatePresence>
